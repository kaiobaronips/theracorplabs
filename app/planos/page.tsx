  ...
  return (
    <section className="py-16 md:py-20">
      <div className="mx-auto w-full max-w-[1100px] px-4 md:px-8">
        <h1 className="text-4xl font-bold leading-tight tracking-[-0.02em] text-tc-gray-900 md:text-5xl">Planos</h1>
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-3">
          {plans.map((plan) => (
            <article key={plan.title} className="tc-card">
              <h2 className="text-xl font-semibold text-tc-gray-900">{plan.title}</h2>
              <p className="mt-3 text-sm text-tc-gray-500">{plan.description}</p>
            </article>
          ))}
        </div>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-4">
          ...
          <Link href="/como-funciona" className="text-sm text-tc-gray-500 underline-offset-2 hover:text-tc-gray-900 hover:underline">Entender a jornada</Link>
        </div>
