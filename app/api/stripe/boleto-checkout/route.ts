...
export async function POST(req: NextRequest) {
    ...
    return NextResponse.json(
      {
        ...
        message: 'Informe um e-mail válido para gerar o boleto.',
      },
      { status: 400 }
    );
  }
  ...
  params.set('payment_method_types[0]', 'boleto');
  ...
  params.set('metadata[source]', 'theracorp_boleto_checkout');
  params.set('metadata[contact_email]', email);
  params.set('subscription_data[metadata][plan]', planName);
  params.set('subscription_data[metadata][source]', 'theracorp_boleto_checkout');
  params.set('subscription_data[metadata][contact_email]', email);
  params.set('success_url', `${origin}/?checkout=boleto-success`);
  params.set('cancel_url', `${origin}/?checkout=boleto-cancel`);
  ...
  if (!response.ok) {
    console.error('[stripe/boleto-checkout]', data);
    const stripeError = data?.error;
    return NextResponse.json(
      {
        error: 'boleto_checkout_failed',
        code: stripeError?.code || stripeError?.type || 'stripe_error',
        message: stripeError?.message || 'Falha ao criar o checkout de boleto.',
      },
      { status: 502 }
    );
  }
